...
export default function PhoenixBlockConfig({ module, pageId }: PhoenixBlockConfigProps) {
  // Get theme
    ...
    if (index !== -1) {
      ...
      if (indexChild !== -1) {
        if (values.pages[index].modules[indexChild].config.columns) {
          values.pages[index].modules[indexChild].config.columns.push({ id: uuid_v4(), modules: [] })
        } else {
          values.pages[index].modules[indexChild].config = { columns: [{ id: uuid_v4(), modules: [] }] }
        }
      }
    }
    setTheme(values)
  ...
  return (
    <ModulesConfigTabs pageId={pageId} module={module}>
      {module?.config?.columns?.map(({ id }: any) => (
        <PhoenixBlockConfigItems key={id} itemId={id} columns={module?.config?.columns} setColumns={e => update('columns', e)} />
      ))}
      <Button
        variant="outline"
        onClick={() => newColumn()}
